...
class BerthingTask(Core):
    ...
    def _generate_berth_centers(self, env_ids):
        """为指定环境生成船位中心"""
        # 将每个环境的船位中心固定在环境原点，
        # 确保可视化的矩形与数值计算（碰撞/目标）一致，避免随机偏移导致的错位
    ...
    def get_spawns(
        ...
        # 在船位前方生成起始位置
        angle_range = math.radians(self._task_parameters.spawn_angle_range)
        r = torch.rand((num_resets,), device=self._device) * (rmax - rmin) + rmin
        theta = torch.rand((num_resets,), device=self._device) * angle_range - angle_range/2
