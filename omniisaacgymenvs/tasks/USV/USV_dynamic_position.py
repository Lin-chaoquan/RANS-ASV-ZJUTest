...
class DynamicPositionTask(Core):
    """
    Implements the DynamicPosition task. The robot has to reach a target position."""

    def __init__(
        self,
        task_param: DynamicPositionParameters,
        reward_param: DynamicPositionReward,
        num_envs: int,
        device: str,
    ) -> None:
        super(DynamicPositionTask, self).__init__(num_envs, device)
        # Task and reward parameters
        self._task_parameters = parse_data_dict(DynamicPositionParameters(), task_param)
        self._reward_parameters = parse_data_dict(DynamicPositionReward(), reward_param)
